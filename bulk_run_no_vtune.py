    ...
    for i in range(len(list_of_gguf_files)):
        ...
        if not auto:
            ...
            if "q" in run or "stop" in run or "STOP" in run:
                break
            if run == "Y" or run == "y":
                subprocess.Popen(["python3", "benchmark_model_run.py", pathmain, f".{path_separator[system][1]}{strlist_of_gguf_files[i]}", num_threads, num_tokens_gen, prompt, ">", f"run{number}_{gguf_info[-1][:-5]}.out"],shell=True).wait()
            ...
            subprocess.Popen(["benchmark_model_run.py", pathmain, f".{path_separator[system][1]}{strlist_of_gguf_files[i]}", num_threads, num_tokens_gen, prompt, ">", f"run{number}_{gguf_info[-1][:-5]}.out"],shell=True).wait()
            #os.system(" ".join(["vtune", "-collect", "memory-access", "--result-dir", name_construct, "python3", "benchmark_model_run.py", pathmain, f".{path_separator[system][1]}{strlist_of_gguf_files[i]}", num_threads, num_tokens_gen, prompt, ">", "vtune.out"]))
            out = get_llm_output.get_out_len()
            get_llm_output.bruteforce_update_name(out,"." + path_separator[system][2] + name_construct + path_separator[system][2])
        time.sleep(1)
